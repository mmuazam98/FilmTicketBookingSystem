...
def reg_user():
    ...
    if((email_info.find(constr1)) > 0 and (email_info.find(constr2)) > 0):
        ...
        try:
            ...
            c.execute('''INSERT INTO user(passwd,name,mail) VALUES (?,?,?) ''',
                      (password.get(), username.get(), email.get()))
            Label(screen1, text="Registered sucessfully",
...
def reg_screen(screen):
    ...
    Label(screen1, text="ENTER DETAILS BELOW", bg="grey",
    ...
    Button(screen1, text="REGISTER", width=10,
           height=1, command=reg_user).pack()


conn = sqlite3.connect('theater.db')
c = conn.cursor()
# c.execute("""DELETE FROM jobs""")
fetch_formDetails_sql = """SELECT * FROM receipt;"""
